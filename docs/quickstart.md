...
- [Claude Code CLI](https://claude.ai/code) with an active Max subscription
- git
- Docker + Docker Compose (optional but recommended for Postgres + cockpit container)
...
cd fleetwright
cp .env.example .env
# Set BRIDGE_SECRET to a random string:
...
- Checks Python ≥ 3.11, git, and the `claude` CLI
- Copies `.env.example` → `.env` (won't overwrite an existing `.env`)
- Generates a random `BRIDGE_SECRET` if still at the placeholder value
- Runs `pip install -e .`
...
Copy the example domain config and customize it for your setup:

```bash
cp config/domains.example.yaml config/domains.yaml
# Edit config/domains.yaml: rename "work" and "personal" to match your areas
```

Each domain needs a working directory on your filesystem. Create them:

```bash
mkdir -p ~/fleetwright-domains/work
mkdir -p ~/fleetwright-domains/personal
```

Restart the bridge after changing `domains.yaml`:
...
| `FLEET_MAX_PARALLEL` | `10` | Max parallel fleet runs. |
...
- Strips `ANTHROPIC_API_KEY` from worker environments so workers authenticate via Max subscription
